...
std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>> getMeasurements()
{
    ...
    //获取时间戳小于图片的imu数据，并确保图像前后之间存在imu数据
    while (ros::ok())
    {
        // 边界条件
        if (imu_buf.empty() || feature_buf.empty())
            return measurements;
        //imu最新的数据没有跟上图片的频率，
        if (!(imu_buf.back()->header.stamp.toSec() > feature_buf.front()->header.stamp.toSec() + estimator.td))//readed image clock + td = real image clock (IMU clock)，0
        {
            return measurements;
        }
        if (!(imu_buf.front()->header.stamp.toSec() < feature_buf.front()->header.stamp.toSec() + estimator.td))//imu数据靠后，在开始时
        {
        ...
        // imu数据(多个)，小于图像的多个imu数据
        std::vector<sensor_msgs::ImuConstPtr> IMUs;
        while (!imu_buf.empty() && imu_buf.front()->header.stamp.toSec() < img_msg->header.stamp.toSec() + estimator.td)
        {
...
void process()
{
    while (ros::ok())
    {
        ...
        con.wait(lk, [&]
                 {
            return (measurements = getMeasurements()).size() != 0;//获取一帧图像对应的imu数据
                 });
        lk.unlock();
        ...
        for (auto &measurement : measurements)
        {
            ...
            // 2.IMU pre-integration，imu之前的数据预积分
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                else
                {
                    ...
                    double dt_1 = img_t - current_time;//前一个imu
                    double dt_2 = t - img_t;//后一个imu
                    current_time = img_t;
                    ROS_ASSERT(dt_1 >= 0);
                    ROS_ASSERT(dt_2 >= 0);
            ...
            // 3.2.从激光里程计中获取先验位姿，imu时间戳和图像时间戳最接近，差值小于1/hz的最接近的一个
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_imu     = n.subscribe(IMU_TOPIC,      5000, imu_callback,  ros::TransportHints().tcpNoDelay());//预测，唤醒线程，发表里程计
    ros::Subscriber sub_odom    = n.subscribe("odometry/imu", 5000, odom_callback);//缓存
    ros::Subscriber sub_image   = n.subscribe(PROJECT_NAME + "/vins/feature/feature", 1, feature_callback);//缓存，唤醒线程
