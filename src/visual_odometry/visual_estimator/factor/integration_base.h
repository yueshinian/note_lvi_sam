...
class IntegrationBase
{
  public:
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        //delta_j = ture_j - ideal_j
        //pvq的顺序
        //计算这个雅克比的目的是为了在优化后更新bias后，预积分也要更新，用一阶泰勒近似，根据观测方程x_k+1 = F*x_k + G*N,，得到k+j时刻的雅克比矩阵，根据链式求导法则有J_k+1 = F*J_k
        //k+1时刻对k时刻参数的导数
        if(update_jacobian)
        {
    ...
    {
        //修正k和k+1时刻，pvq2bias，bias修正后会影响预计分的值，一阶泰勒近似
        Eigen::Matrix<double, 15, 1> residuals;//残差=q_bw*param - pre_integration
        //修正预计分的值，因为bias改变，预计分的泰勒展开，所以雅可比是预计分对bias的偏导
        Eigen::Matrix3d dp_dba = jacobian.block<3, 3>(O_P, O_BA);//dp是预计分量
        ...
        residuals.block<3, 1>(O_V, 0) = Qi.inverse() * (G * sum_dt + Vj - Vi) - corrected_delta_v;//e = 
