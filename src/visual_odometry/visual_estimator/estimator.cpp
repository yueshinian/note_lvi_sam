...
//WINDOW_SIZE=10，状态量数组是WINDOW_SIZE+1=11大小的数组，大部分情况下，frame_count = WINDOW_SIZE=10，
// imu预积分，一帧图像处理多帧imu，接近imu频率
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    //frame_count是图像的帧数量，如果当前位置没有初始化，那么就new一个预积分对象
    ...
    {
        // 预积分
        //关键帧——这是11帧的数据，执行滑动窗口后，如果边缘化最老帧，就把11帧数据赋值给10帧，如果边缘化10帧，那么11帧imu数据累加到10帧上，然后delete，初始化
        pre_integrations[frame_count]->push_back(dt, linear_acceleration, angular_velocity);
        //保存两个图像之间之间的imu数据，在处理图像的入口赋值给图像后，就delete，初始化，感觉可以用上面的代替
        tmp_pre_integration->push_back(dt, linear_acceleration, angular_velocity);

        // 保存imu数据，这个起到边缘化第10帧的时候，把当前imu数据再在10帧的基础上，重新积分到11帧
...
// 处理图像数据，image特征点像素2，归一化坐标3，速度2，深度1；里程计信息换成数组存储；处理的图像的header
                             ...
                             const std_msgs::Header &header)
{
    ...
    // 1.添加特征点, 检测视差 (根据视差决定marge最老帧还是次新帧)，跟踪点数量过小或者像素移动距离比共视点数量过大，那么相邻帧共视点过少，边缘化最老帧
    ...
    //tmp_pre_integration的生命周期就到这里，为啥不用数组那个代替，数组累积的是关键帧之间所有imu的数据
    tmp_pre_integration = new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};

    // 3.在线外参标定 (camera to imu 的旋转)
    //imu与积分和两帧图像对积极和求解，得到两条路，当前imu位子到上一帧camera位子，多个方程，超定方程
    // 3.Calibrate rotational extrinsics
    if(ESTIMATE_EXTRINSIC == 2)
    {
        ROS_WARN("calibrating extrinsic param, rotation movement is needed");
        if (frame_count != 0)//一帧不可标定
        {
            // 前后两帧的归一化坐标
            vector<pair<Vector3d, Vector3d>> corres = f_manager.getCorresponding(frame_count - 1, frame_count);//前后两帧公式点，归一化平面点
            Matrix3d calib_ric;
            // 标定camera to imu的旋转，k和k+1帧的图像和imu数据，将k+1刻imu坐标变换到k时刻camera坐标，有两条路可走
            //失败条件：需要标定10次，或者因为基于SVD分解求取R，为了让误差最小，取得是最后一列向量，其对应的奇异值要够大，才证明标定的有效性
...
bool Estimator::initialStructure()
{
    ...
    // 2.通过加速度标准差来判断imu是否有效
    {
        ...
        for (frame_it = all_image_frame.begin(), frame_it++; frame_it != all_image_frame.end(); frame_it++)
        {
            double dt = frame_it->second.pre_integration->sum_dt;
            Vector3d tmp_g = frame_it->second.pre_integration->delta_v / dt;
            var += (tmp_g - aver_g).transpose() * (tmp_g - aver_g);//方差之和
    ...
    struct SFMFeature
{
    bool state; // 是否被三角化
    int id; //特征点id
    vector<pair<int,Vector2d>> observation;//观测到的图像帧，以及像素坐标
    double position[3];//归一化坐标
    double depth;//深度
};
    */
    ...
    Vector3d relative_T;
    int l;
    // 寻找参考帧, 并计算相对pose (在滑动窗口内, 寻找与最新帧有足够视差并且有足够内点的frame, 作为sfm的参考帧)，本质矩阵分解效果够好，
    if (!relativePose(relative_R, relative_T, l))
    {
        ROS_INFO("Not enough features or parallax; Move device around");
        return false;
    }
    //当前帧和参考帧三角化可以同时观察到的点，还原特征点深度，然后pnp求解可以观察到该点的帧的位姿
    //pnp求解图像位子，三角化求特帧点深度，BA优化
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        // 在滑动窗口内, 不需要再计算位姿
        if((frame_it->first) == Headers[i].stamp.toSec())//Headers存储的是关键帧的时间戳，和它时间戳相同，说名就是关键帧
        {
            frame_it->second.is_key_frame = true;
            frame_it->second.R = Q[i].toRotationMatrix() * RIC[0].transpose();//旋转相机坐标系到imu坐标系，Rcc*Rci
        ...
        */
        for (auto &id_pts : frame_it->second.points)//遍历该图像帧对应的所有特征点,int,vector<pair<int, Eigen::Matrix<double, 8, 1>>>
        {
            int feature_id = id_pts.first;
            for (auto &i_p : id_pts.second)//pair<int, Eigen::Matrix<double, 8, 1>>，，，，vector<pair<int, Eigen::Matrix<double, 8, 1>>>
            {
                it = sfm_tracked_points.find(feature_id);
                if(it != sfm_tracked_points.end())
                {
                    Vector3d world_pts = it->second;
                    cv::Point3f pts_3(world_pts(0), world_pts(1), world_pts(2));//被三角化的点世界坐标系，初始的相对世界
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)
        {
            ...
            // 视差大于30, 并且内点数大于12
            if(average_parallax * 460 > 30 && m_estimator.solveRelativeRT(corres, relative_R, relative_T))//计算本质矩阵和Rt
            {
                l = i; // 第i帧为参考帧
...
void Estimator::solveOdometry()
{
    ...
    {
        f_manager.triangulate(Ps, tic, ric);//三角化新的全部可以三角化的点
        optimization();
    }
}
...
// 非线性优化
//相机15维度，相机外参6维度，特征点1维度（相对于起始帧的深度），故一共15*n+6+1*m个维度
//目标函数，边缘化残差，imu残差，视觉残差
...
//添加参数块，n*15+5+1*m个维度
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        problem.AddResidualBlock(imu_factor, NULL, para_Pose[i], para_SpeedBias[i], para_Pose[j], para_SpeedBias[j]);//15x7,15x9
    }
    ...
    for (auto &it_per_id : f_manager.feature) // 遍历所有features
    {
        ...
        ++feature_index;//为了与这个索引保持一致para_Feature
    ...
    // 10.边缘化处理
    //边缘化的目的是什么？它的目的只有一个，在下次优化的时候提供一个约束，提供一个残差，
    //那么它迭代需要什么信息？高斯牛顿法，需要雅可比矩阵和残差，残差ep = e0 + J^T * dx,J不变
    //所以这步边缘化的目的就是提供一个初始残差和雅可比矩阵，怎么算？
    //构造H*[x1 , x2]^T = b这样的矩阵，x1是需要边缘化信息，包括0帧和上次边缘化的信息，通过舒尔补施加给x2,构造新的H*x2=b，通过SVD分解可以求出e0和J
    //下一次迭代仅需要这两个东西，那么直观的表示是什么？
    //就是矩阵叠加，J^T*T的叠加，各个海赛矩阵的叠加，都有哪些？
    //上一次的边缘化矩阵，imu约束，视觉约束，都该怎么算？
    //上一次的边缘化矩阵直接叠加，imu和视觉都自定义了雅可比残差，我们new一个factor，然后调用它的Evaluate函数，就可以算出它的雅可比和残差，直接赋值给矩阵对应的位置
    //所以流程是这样的，
    /*
        marginalization_info总管这次的边缘化工作，简称marg_info
        根据约束关系，先new一个factor，然后marg_info调用add_ResidualBlockInfo,构造地址--size的map，构造边缘化的地址--id
        preMarginalize计算雅可比和残差，构造地址--data
        marginalize构造地址-id，多线程计算H和b，舒尔补计算残差和初始雅可比
    */
    if (marginalization_flag == MARGIN_OLD)
    {
        // ----------------------10.1.边缘化最老帧-----------------------
        ...
        // 10.1.1.上一轮的边缘化先验信息,第一次没有边缘化
        //假设上次边缘化了0帧，和1帧有约束，则提取1帧的信息
        if (last_marginalization_info)
        {
            vector<int> drop_set;
        ...
        // 10.1.2.将被marge的imu残差信息，imu中边缘化0帧的pvq和bias，影响1帧的pvq和bias
        {
        ...
        // 10.1.3.将被marge的视觉残差信息，起始帧为0帧的特征点信息，边缘化0帧PQ，影响特征点关联的其他帧关系
        {
            int feature_index = -1;
            for (auto &it_per_id : f_manager.feature) // 遍历features，起始帧是第0帧，而且被观测2次以上
            {
                it_per_id.used_num = it_per_id.feature_per_frame.size();
                if (!(it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2))
        ...
        // 10.1.4.计算残差，计算映射地址——数据，这个映射
        TicToc t_pre_margin;
        marginalization_info->preMarginalize();
        ROS_DEBUG("pre marginalization %f ms", t_pre_margin.toc());
        
        // 10.1.5.边缘化，计算地址——矩阵起始位置，这个映射，构建Hx=b,舒尔补计算
        TicToc t_margin;
        ...
        //获得受到影响的参数块，n*n
        vector<double *> parameter_blocks = marginalization_info->getParameterBlocks(addr_shift);//更新边缘化内容
        //更新
        if (last_marginalization_info)
            delete last_marginalization_info;
...
void Estimator::slideWindow()
{
    ...
    {
        // 2.marge掉次新帧，去掉视觉约束，保留imu数据约束的连贯性
