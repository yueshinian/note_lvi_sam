...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image, double td)
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        if (it_per_id.start_frame <= frame_count - 2 &&
            it_per_id.start_frame + int(it_per_id.feature_per_frame.size()) - 1 >= frame_count - 1)//0-10帧，检测8和9帧视差
        {
            // 倒数第三帧 和 倒数第二帧 之间的视差
...
//十四讲s2*x2 = R*s1*x1 + t，s2_hat*R*s1*x1+s2_hat*t=0，RT是两副图像之间的位置姿态变化，用imu数据来代替，求解尺度s2
//这里将c0看作世界坐标系，R01 = Rw0^-1 * Rw1，由x=TX，这里得到的AX=0，求得的坐标其实是相对于起始帧相机坐标系的xyz，深度值也是起始帧坐标系的深度
// 三角化features的深度，相机位置姿态，相机外参
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        it_per_id.used_num = it_per_id.feature_per_frame.size();
        if (!(it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2))//最少被两个关键帧观测到，10-2=8，而且起始帧在0-7帧之间，也就是0-9帧之间
            continue;
        //之前估计过了，也跳过
        ...
        Eigen::MatrixXd svd_A(2 * it_per_id.feature_per_frame.size(), 4);//缺了深度，减少一个维度，所以每个特征点提供两个约束方程
        int svd_idx = 0;
        ...
        //将c0看作世界坐标系，R01 = Rw0^-1 * Rw1，这里得到的AX=0，求得的坐标其实是相对于起始帧相机坐标系的xyz，深度值也是起始帧坐标系的深度
...
void FeatureManager::removeBackShiftDepth(Eigen::Matrix3d marg_R, Eigen::Vector3d marg_P, Eigen::Matrix3d new_R, Eigen::Vector3d new_P)
{
         ...
         it != feature.end(); it = it_next)
    {
        ...
        else
        {
            ...
            it->feature_per_frame.erase(it->feature_per_frame.begin());//删除起始帧
            //观测数量少于2，删除
            ...
            {
                Eigen::Vector3d pts_i = uv_i * depth; // feature in cartisian space in old local camera frame，0帧坐标系
                Eigen::Vector3d w_pts_i = marg_R * pts_i + marg_P; // feautre in cartisian space in world frame，世界坐标系
                Eigen::Vector3d pts_j = new_R.transpose() * (w_pts_i - new_P); // feature in cartisian space in shifted local camera frame，1帧坐标系
...
void FeatureManager::removeFront(int frame_count)
{
    for (auto it = feature.begin(), it_next = feature.begin(); it != feature.end(); it = it_next)
    {
        ...
        {
            int j = WINDOW_SIZE - 1 - it->start_frame;//10-1-9
            if (it->endFrame() < frame_count - 1)//  <10-1，即第10帧没有观测到
                continue;
            it->feature_per_frame.erase(it->feature_per_frame.begin() + j);//删除特征包含了这帧的信息
            if (it->feature_per_frame.size() == 0)//没有观测帧，删除该帧
                feature.erase(it);
        }
    }
}

// 计算特征点的视差 (倒数第二帧 和 倒数第三帧 之间的视差），一共11帧，第9和第10帧之间的共视点在归一化平面上的距离差
