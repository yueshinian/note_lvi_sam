			  ...
			  vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
	...
	// 对于参考帧和最新帧之间的每一帧, 先进行pnp求解相机位姿, 再对特征点进行三角化 (第i帧与最新帧)
	// 循环结束后, 最新帧的大部分特征点都被三角化了
	//三角化l和最新帧的共视点，求解l+1帧的位姿PNP，三角化l+1帧和最新帧特征点，那么l+1帧大概率起始帧
	for (int i = l; i < frame_num - 1 ; i++)
	{
		...
		// 1: 三角化特征点 (第i帧与最新帧)
		triangulateTwoFrames(i, Pose[i], frame_num - 1, Pose[frame_num - 1], sfm_f);//三角化出的特征点都是以参考帧为世界坐标系
	}

	// 3: triangulate l-----l+1 l+2 ... frame_num -2
	// 3: 对于参考帧和最新帧之间的每一帧, 对特征点进行三角化 (第i帧与参考帧)
	// 3: 循环结束后, 参考帧的大部分特征点都被三角化了
	//三角化l+1到当前帧的，和参考帧有共视点的特征点
...
*/
	...
	for (int i = 0; i < frame_num; i++)
	{
		...
		if (i == l)
		{
			problem.SetParameterBlockConstant(c_rotation[i]);//固定参考帧的位姿
		}
		if (i == l || i == frame_num - 1)
		{
			problem.SetParameterBlockConstant(c_translation[i]);//固定参考帧的平移
		}
	}
	// 6.2: 特征点坐标
