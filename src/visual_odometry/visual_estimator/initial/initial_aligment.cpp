...
//之前已经将图像相邻帧的旋转变换转为imu坐标系下，相机相邻两帧变换和imu相邻两帧变换的乘积应当是个四元数，因此q_c_k^-1 * q_c_k+1 * q_imu_k2k+1 = [1 0]^T
//但是imu有误差，即bias，故q_imu_k2k+1_true = q_imu_k2+1_ideal*[1 J*delta_w/2.0]^T，求得就是delta_w
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    // 遍历所有frames, 每两个相邻的frame构成一个约束，两帧之间图像的R和imu的R构成约束
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    // 3.重力细化 (固定重力的模长, 优化速度, 重力和尺度)，利用重力莫长
