...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
   ...
   if (PUB_THIS_FRAME)
   {
        ...
        *depth_cloud_temp = *depthCloud; // 5s内的激光点云
        ...
        // 4.在图像上标记提取到的特征点, 发布出去用于显示，useless
...
void lidar_callback(const sensor_msgs::PointCloud2ConstPtr& laser_msg)
{
    ...
    *laser_cloud_in = *laser_cloud_in_ds;
    ...
    for (int i = 0; i < (int)laser_cloud_in->size(); ++i)
    {
        ...
        if (p.x >= 0 && abs(p.y / p.x) <= 10 && abs(p.z / p.x) <= 10)//todo
...
int main(int argc, char **argv)
{
    ...
    pub_feature = n.advertise<sensor_msgs::PointCloud>(PROJECT_NAME + "/vins/feature/feature",     5); // 当前帧的特征，发给位姿估计
    pub_match   = n.advertise<sensor_msgs::Image>     (PROJECT_NAME + "/vins/feature/feature_img", 5); // 当前帧的图像 (标记特征点)，可视化，useless
