...
public:
                                          ...
                                          const vector<geometry_msgs::Point32>& features_2d) // 去畸变后的归一化坐标 xy1
    {
        ...
        // 0.2  check if depthCloud available空的直接返回
        if (depthCloud->size() == 0)
            return depth_of_point;

        // 0.3 look up transform at current image time//camera to world transform
        try{
        ...
        float bin_res = 180.0 / (float)num_bins; // currently only cover the space in front of lidar (-90 ~ 90)，num_bins=360
        cv::Mat rangeImage = cv::Mat(num_bins, num_bins, CV_32F, cv::Scalar::all(FLT_MAX));

        for (int i = 0; i < (int)depth_cloud_local->size(); ++i)//激光点云
        {
            PointType p = depth_cloud_local->points[i];
            // filter points not in camera view
            if (p.x < 0 || abs(p.y / p.x) > 10 || abs(p.z / p.x) > 10)//todo滤除标准？
                continue;
            // find row id in range image
            float row_angle = atan2(p.z, sqrt(p.x * p.x + p.y * p.y)) * 180.0 / M_PI + 90.0; // degrees, bottom -> up, 0 -> 360，-90～90度的点云，todo
            int row_id = round(row_angle / bin_res);
            // find column id in range image
            float col_angle = atan2(p.x, p.y) * 180.0 / M_PI; // degrees, left -> right, 0 -> 360，-90～90度的点云，转到0-180度
            int col_id = round(col_angle / bin_res);//再乘以2，实际只有90度的点云
        ...
        *depth_cloud_local = *depth_cloud_local_filter2;
        ...
        // 7. find the feature depth using kd-tree，todo
        ...
        for (int i = 0; i < (int)features_3d_sphere->size(); ++i)
        {
            ...
            if (pointSearchInd.size() == 3 && pointSearchSqDis[2] < dist_sq_threshold)
            {
                ...
                //todo原点和相机点确定的直线，和ABC确定的平面相交，原点到交点的距离
                ...
                //两个条件，第一个是不同时间的深度点云叠加，通过判断最大最小距离之差，第二个是点到平面的距离0.5，因为点云最小距离是0.5
                if (max_depth - min_depth > 2 || s <= 0.5)//最大最小距离差距过大，或者距离小于0.5，都不可靠
                {
                    continue;
