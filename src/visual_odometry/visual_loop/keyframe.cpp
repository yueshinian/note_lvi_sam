		           ...
		           vector<double> &_point_id)
{
    ...
    // 对滑动窗口内的特征点, 计算描述子，计算已有特征点
	computeWindowBRIEFPoint();
    // 提取新的FAST角点, 再计算描述子,重新提取特征点
	computeBRIEFPoint();
	if(!DEBUG_IMAGE)
		image.release();
}
                                ...
                                const std::vector<cv::KeyPoint> &keypoints_old_norm)
{
    for(int i = 0; i < (int)window_brief_descriptors.size(); i++)
    {
        ...
        if (searchInAera(window_brief_descriptors[i], descriptors_old, keypoints_old, keypoints_old_norm, pt, pt_norm))
          status.push_back(1); // 匹配成功
        else
          status.push_back(0); // 匹配失败
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	matched_3d = point_3d;               // 3d坐标
	matched_2d_cur = point_2d_uv;        // 2d坐标
	matched_2d_cur_norm = point_2d_norm; // 归一化坐标
	matched_id = point_id;               // index
